		...
		<DIV class=list_left>
			...
			<DIV class=sub_nav>
				...
				<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
  					<TBODY>
  						<TR>
    						...
    						<TD class=sub_nav_tit_main><DIV class=sub_nav_ico><A href="index.jsp">首页</A></DIV></TD>
    			...
    			<DIV class=sub_nav_main>
    				...
    				<TABLE class=class_td width="95%" align=center>
  						<TBODY>
						  	<TR>
						  		<TD><A class=linktext href="AskSearch.do?act=search&status=0_1&typeid=5&orderby=expiredTime desc">创业问答</A></TD>
						    	<TD><A class=linktext href="AskSearch.do?act=search&status=0_1&typeid=6&orderby=expiredTime desc">综合问答</A></TD>
						    	<TD><A class=linktext href="AskSearch.do?act=search&status=0_1&typeid=1&orderby=expiredTime desc">投资问答</A></TD>
						    	<TD><A class=linktext href="AskSearch.do?act=search&status=0_1&typeid=3&orderby=expiredTime desc">融资问答</A></TD>
						    	<TD><A class=linktext href="AskSearch.do?act=search&status=0_1&typeid=4&orderby=expiredTime desc">理财问答</A></TD>
			...
			<DIV class=list_main>
				<DIV class=list_main_tit_1 id=tabmenu1>
					<UL>
						<LI <c:if test="${PARA['flag'] == 'newest' }">class=on</c:if>><A href="AskSearch.do?act=search&status=0_1&keyword=&flag=newest&orderby=expiredTime desc">最新问题</A> </LI>
						<LI <c:if test="${PARA['flag'] == 'hard' }">class=on</c:if>><A href="AskSearch.do?act=search&status=0&keyword=&flag=hard&orderby=expiredTime desc">待解决问题</A> </LI>
						<LI <c:if test="${PARA['flag'] == 'reward' }">class=on</c:if>><A href="AskSearch.do?act=search&status=0_1&keyword=&flag=reward&orderby=reward desc">高积分问题</A> </LI>
						<!-- 
						<LI <c:if test="${PARA['flag'] == 'vote' }">class=on</c:if>><A href="AskSearch.do?act=search&status=0_1&keyword=&flag=vote&orderby=expiredTime desc">投票</A> </LI>
						 -->						
						<LI <c:if test="${PARA['flag'] == 'resolved' }">class=on</c:if>><A href="AskSearch.do?act=search&status=1&keyword=&flag=resolved&orderby=expiredTime desc">已解决问题</A> </LI>
					</UL>
					<DIV class=clear></DIV>
				</DIV>
				...
				<DIV class=tabmain id=tabmains1>
  							...
  							<DIV class=list_con>
  								<DIV class=list_main_tit_2>
							  		<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
							    		<TBODY>
							    			<TR>
									      		<TD width="5%">&nbsp;</TD><!--<td width="13%">状态</td>-->
									      		<TD width="67%">标题</TD>
									      		<TD width="8%">回答</TD>
									      		<TD width="10%">悬赏</TD>
									      		<TD width="10%">提问时间</TD>
								...
								<DIV class=list_con_all>
  									<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
								    	...
								    	<c:forEach items="${ASK_LIST}" var="ask"> 
									    	<TR><!--<td width="7%" class="wrong_ico">&nbsp;</td>-->
									      		...
									      		<TD class=xuanshang_ico width="12%">${ask.reward}</TD>
									      		<TD width="9%"><fmt:formatDate value='${ask.expiredTime}' pattern='MM-dd' /></TD>
									      	</TR>
								    	</c:forEach>
   										</TBODY>
   									</TABLE>
  								...
  								<DIV class=list_search>
  									...
  									<FORM name=searchask2 action="AskSearch.do?act=search&orderby=expiredTime desc" method=post>
  									<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>    
    									<TBODY>
    									<TR>
								      		<TD class=list_search_img width="13%">&nbsp;</TD>
								      		<TD width="36%"><INPUT class=list_search_input size=45 name=keyword></TD>
								      		<TD width="16%">
								      			<SELECT class=input onchange=ac() name=status> 
								          			<OPTION value=1 selected>已解决问题</OPTION> 
								          			<OPTION value=0>未解决问题</OPTION>
								          		</SELECT>
								          	</TD>
								      		<TD width="17%"><INPUT class=list_search_btn_1 type=submit src="images/list_search_btn_1.gif" value=" "></TD>
								      		<TD width="18%"><INPUT class=list_search_btn_2 onClick="window.open('ask.jsp')" type=button value=" "></TD>
										</TR>
								    	</TBODY>
								    </TABLE>
								    </FORM>
					...
					<!--<ul><div class="list_con">我是投票中列表</div></ul>
			 		...
			 		<ul><div class="list_con">我是全部列表</div></ul>--></DIV>
