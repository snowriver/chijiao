...
function DelStory(bid){
	if(window.confirm("你确实要删除这本图书么？")){
		location.href='action/story_do.php?bid='+bid+'&action=delbook';
	}
}

function DelStoryContent(cid){
	if(window.confirm("删除内容后章节的其它内容排列序号不会发生变化，\r\n这可能导致管理混乱，你确实要删除这篇内容么？")){
		location.href='action/story_do.php?cid='+cid+'&action=delcontent';
	}
}
...
function EditChapter(cid){
	...
	myajax.SendGet2('action/story_do.php?cid='+cid+'&chaptername='+chaptername+'&chapnum='+chapnum+'&action=editChapter');
	DedeXHTTP = null;
}
...
function DelStoryChapter(cid){
	if(window.confirm("删除章节会删除章节下的所有内容，你确实要删除么？")){
		location.href='action/story_do.php?cid='+cid+'&action=delChapter';
	}
}
...
function DelAllBooks()
{
	if(window.confirm("你确实要删除这些图书么？")){
		...
		location.href='action/story_do.php?bid='+selbook+'&action=delbook';
	}
}
