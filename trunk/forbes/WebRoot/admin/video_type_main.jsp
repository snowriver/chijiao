...
function CommonMenu(obj,tid,returnUrl)
{
  ...
  popupoptions = [
    ...
    new ContextItem("删除栏目",function(){ location="AdminUpdateVideoType.do?act=delete&id="+tid+"&returnUrl="+returnUrl; }),
    new ContextSeperator(),
...
<div class="bodytitle">
	...
	<div class="bodytitletxt">视频类别管理</div>
</div>

<table width="96%" border="0" cellpadding="1" cellspacing="1" align="center" class="tbtitle" style="	background:#E2F5BC;">
	<tr>
		<td bgcolor="#EDF9D5" class="tbtitletxt">
			<div style='float:left'><strong>视频类别管理</strong>(在IE浏览器中可使用右键菜单进行操作。)</div>
			<div style='float:right;padding-right:30px'><a href='video_type_add.jsp?pid=0'><b>↑创建视频类别</b></a></div>
		</td>
	</tr>
</table>
...
<table width='96%' border='0' cellpadding='1' cellspacing='0' align='center' style='margin:0px auto' class='tblist2'>
	<tr align='center' oncontextmenu="CommonMenu(this,${at.id},'|AdminUpdateVideoType.do|act=list|pno=${PAGER.curPage}|')">
		...
		<td width='34%' align='center' style='letter-spacing:1px;'>
			...
			| <a href='video_type_add.jsp?pid=${at.id }'>添加</a>
			| <a href='AdminUpdateVideoType.do?act=get&id=${at.id }&returnUrl=|AdminUpdateVideoType.do|act=list|pno=${PAGER.curPage}|'>修改</a>
			| <a href='AdminUpdateVideoType.do?act=move_get&id=${at.id }&returnUrl=|AdminUpdateVideoType.do|act=list|pno=${PAGER.curPage}|'>移动</a>
			| <a href='＃' onclick="return deleteConfirm('AdminUpdateVideoType.do?act=delete&id=${at.id}','|AdminUpdateVideoType.do|act=list|pno=${PAGER.curPage}|','此操作不能恢复！！！请慎重！！！\n\n确定要删除该文章类型吗？');">删除</a>
		</td>
...
<c:forEach items="${at.videoTypes}" var="a" varStatus="is">
	<table width='96%' border='0' cellspacing='1' cellpadding='0' align='center' style='margin:0px auto' class='tblist2'>
		<tr align='center' class='trlbg'  oncontextmenu="CommonMenu(this,${a.id},'|AdminUpdateVideoType.do|act=list|pno=${PAGER.curPage}|')">
			...
			<td width='34%' align='center' style='letter-spacing:1px;'>
				...
				<a href='AdminUpdateVideoType.do?act=get&id=${a.id }&returnUrl=|AdminUpdateVideoType.do|act=list|pno=${PAGER.curPage}|'>修改</a> | 
				<a href='AdminUpdateVideoType.do?act=move_get&id=${a.id }&returnUrl=|AdminUpdateVideoType.do|act=list|pno=${PAGER.curPage}|'>移动</a> | 
				<a href='#' onclick="return deleteConfirm('AdminUpdateVideoType.do?act=delete&id=${a.id}','|AdminUpdateVideoType.do|act=list|pno=${PAGER.curPage}|','此操作不能恢复！！！请慎重！！！\n\n确定要删除该文章类型吗？');">删除</a> 
			</td>
...
<table width="96%" border="0" cellpadding="5" cellspacing="1" align="center" style=" border:1px solid #E2F5BC;line-height:31px;margin:auto;">
	...
	<tr>
        <td height="20" bgcolor="#EDF9D5" align="center" valign="middle">
			...
			<A href="AdminUpdateVideoType.do?act=list&pno=1">首页</A>&nbsp;&nbsp;
        	<A href="AdminUpdateVideoType.do?act=list&pno=${PAGER.prePage}">上页</A>&nbsp;&nbsp;
        
			<SELECT onchange="selectOnchange(this.options[this.options.selectedIndex].value, 'AdminUpdateVideoType.do?act=list')"> 
        	...
        	<A href="AdminUpdateVideoType.do?act=list&pno=${PAGER.nextPage}">下页</A>&nbsp;&nbsp;
        	<A href="AdminUpdateVideoType.do?act=list&pno=${PAGER.totoalPage}">末页</A>&nbsp;&nbsp;
		</td>
    </tr>
    
	<tr>
		<td height="35" align="center" style="background:#F9FFE6;padding:7px 6px 6px 10px;" >
			<input type="button" value="增加顶级栏目" class="inputbut" onclick="location='video_type_add.jsp?pid=0';"/>
