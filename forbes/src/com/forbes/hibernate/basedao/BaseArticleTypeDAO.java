...
/**
 * A data access object (DAO) providing persistence and search support for ArticleType
 ...
 * 
 * @see com.forbes.hibernate.bean.ArticleType
 * @author MyEclipse Persistence Tools
 */

public class BaseArticleTypeDAO extends HibernateDaoSupport {
	private static final Log log = LogFactory.getLog(BaseArticleTypeDAO.class);
	// property constants
	...
	public ArticleType findById(java.lang.Integer id) {
		...
		try {
			ArticleType instance = (ArticleType) getSession().get(
					"com.forbes.hibernate.bean.ArticleType", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(ArticleType instance) {
		...
		try {
			List results = getSession().createCriteria(
					"com.forbes.hibernate.bean.ArticleType").add(Example.create(instance))
					.list();
	...
	public ArticleType merge(ArticleType detachedInstance) {
		...
		try {
			ArticleType result = (ArticleType) getSession().merge(detachedInstance);
