...
<DIV id="not_solve_question">
		...
		<c:if test="${empty NOT_SOLVE_ASK_LIST}">
		无
		</c:if>
...
<DIV class=question_list_table>
	...
	<DIV id="height_solve_question">
		...
		<c:if test="${empty HEIGHT_REWARDE_ASK_LIST}">
		无
		</c:if>
...
<DIV id="already_solve_question">
		...
		<c:if test="${empty ALREADY_SOLVE_ASK_LIST}">
		无
		</c:if>
	...
	<DIV class=jifen>
		...
		<DIV style="LINE-HEIGHT: 22px; PADDING-LEFT: 10px" class="jinji_main hottop">
			<DIV id="pop_question">
				...
				<c:if test="${empty POP_ASK_LIST}">
				无
				</c:if>
				
				<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%">
					<TBODY>
						<c:forEach items="${POP_ASK_LIST}" var="ask"> 
						<TR>
				    		<TD>
