...
<script language="javascript" type="text/javascript">
	<!--
		function pageselectCallback(page_id, jq){
		  ...
		  window.location="ClientManageVideoFavorite.do?act=list&pno=" + (page_id+1);
		...
		function deleteVideoFavorite(fid, pid){
				$.ajax({
					beforeSend: function(){
				     //$("#RESULTE_MSG").html("删除中...");
				   },
					url: 'ClientManageVideoFavorite.do',
					type: 'post',
					dataType: 'html',
...
<DIV class=mainbody>
	...
	<DIV class=data_opt_title>
		<UL>
  			...
  			<LI><A class=curmenu>视频收藏</A> </LI>
  			<LI><A class=ocurmenu href="ClientManageNewsFavorite.do?act=list">资讯收藏</A> </LI>
  		</UL>
  	...
  	<DIV style="PADDING-LEFT: 2%">
		...
		<TABLE class=datalist_opt border=0 cellSpacing=0 cellPadding=0 width="95%">
			<TBODY>
		    	...
		    	<c:if test="${empty VIDEO_FAVORITE_LIST}">
		    		<TR>
		    			<TD colspan="5" align="center">您还没有视频收藏！</TD>
		    		</TR>
		    	</c:if>
		    	
		    		<c:forEach items="${VIDEO_FAVORITE_LIST}" var="fav" varStatus="is">
		    		<TR>
			    		<TD align=middle><INPUT id=articleid value=${fav[0].id } type=checkbox name=articleid></TD>
			    		<TD style="WIDTH:50px">视频收藏</TD>
			    		...
			    		<TD align="center">
			    			<A href="javascript:void(0);" onclick="deleteVideoFavorite(${fav[1].id },${PAGER.curPage-1});">删除</A>
			      		</TD>
			      	</TR>
		      		</c:forEach>
